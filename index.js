...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛TO CARREGANDO O BAGUI ESPERA AI SE DEMORAR UM POUCO PE�A DNV🤯⌛',
				success: '✔️Sucesso✔️',
				...
				only: {
					...
					Badmin: '[❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados Do  AD1206P tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar bruxinho|19`,
				}
			}
			...
			const ownerNumber = ["558499278703@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '558499278703@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marca quem tu quer promover burro do krlh� !')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Aqui está o número do meu dono, salve ae poha',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
